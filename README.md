...
- Date picker (1 to 31): tag each invoice with its day of the month; the list auto-sorts by date
- Drag to reorder: fine-tune order with a grip handle; manual order is preserved across auto-sorts
- Processed checkbox: strike through invoices as you finish them
- Filename filter: quickly narrow the list
- Resizable sidebar: adjust the file list width
...
| `Insort_x.y.z_x64_portable.exe` | Portable, no install, just run |
...
3. Use the calendar button to set each invoice's date (1 to 31). The list re-sorts by date automatically.
4. Drag rows by the grip handle to fine-tune order (for example, same-date invoices).
...
- **Bootstrap 5.3** and **SortableJS** vendored locally (fully offline)
